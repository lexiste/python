...
vti_api_key = '0c0f486c881054366010d8c294b6c935d4efb5357dcdb3da8c69ce5841654075'  # put your VirusTotal API key here
vti_upload_url = 'https://www.virustotal.com/vtapi/v2/file/scan'
vti_comment_url = 'https://www.virustotal.com/vtapi/v2/comments/put'
bamfdetect = '/root/logs/src/bamfdetect/bamfdetect'
pastes_dir = '/root/logs/pastes/'
done_dir = '/root/logs/pastes/done/'
rsrch_dir = '/root/logs/pastes/research/'
...
myResolver.nameservers = ['8.8.4.4', '8.8.8.8']  # edit for local DNS nameservers
#config = {}
...
def tweet(status):
    try:
        twitter = Twitter(auth = OAuth(config["a_key"], config["a_sec"], config["c_key"], config["c_sec"]))
...
#####
# 2019-07-30 -- loading my login data for systems
#####
try:
    with open("/root/scripts/login.json", "r") as secrets:
        data = json.load(secrets) # data should have all our elements, pick what we need
        tw_ak = data['twitter_access']['a_key']
        tw_as = data['twitter_access']['a_sec']
        tw_ck = data['twitter_consumer']['c_key']
        tw_cs = data['twitter_consumer']['c_sec']
        secrets.close()
except IOError as e:
    ErrorOutput("Could not access the login.json file to extract keys")
