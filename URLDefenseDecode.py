...
# https://help.proofpoint.com/@api/deki/files/177/URLDefenseDecode.py?revision=2
#
...
class URLDefenseDecoder(object):
    ...
    def decode(self, rewritten_url):
        ...
        #print("version matched:", match.group(1))
        if match:
            if match.group(1) == 'v1':
    ...
    def decode_v2(self, rewritten_url):
        ...
        #print("v2 url passwd: ", rewritten_url)
        if match:
            special_encoded_url = match.group('url')
            #print("base url: ", special_encoded_url)
            trans = maketrans('-_', '%/')
            url_encoded_url = special_encoded_url.translate(trans)
            #print("encoded url: ", url_encoded_url)
            html_encoded_url = unquote(url_encoded_url)
            #print("html url: ", html_encoded_url)
            url = unescape(html_encoded_url)
            return url
        else:
            raise ValueError('Error parsing URL')
